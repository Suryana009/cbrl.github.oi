    ...
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                <div class="panel-heading">Data @lang('general.solution')</div>

                <div class="panel-body">
                    <div class="table-responsive">
                        <a class="btn btn-success" href="{{ url('solution/create') }}"> @lang('general.create')</a>
                        <br>
                        <br>
                        <table class="table table-bordered" id="tbl_solution" style="font-size: 12px;">
                            {{ csrf_field() }}
                            <thead>
                                <tr>
                                    <th class="text-center" style="width: 7%;">#</th>
                                    <th>@lang('general.disease')</th>
                                    <th>@lang('general.solution')</th>
...
$(document).ready(function(){
    ...
    table = $('#tbl_solution').DataTable({ 
        "processing": true,
        "serverSide": true,
        "order": [],
        "ajax": {
            "url": "{{ url('solution/show_solution_DT') }}",
            data: {_token:token},
            "type": "POST"
        },
        "columnDefs": [
            ...
            $.ajax({
                ...
                error: function() {
                    Swal.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                url: "{{ url('solution/unactivate') }}/"+id,
                                success: function() {
                                    Swal.fire({
